...
- [ ] **EVO35.3d** [SEED_V3_VISUAL_PIPELINE_RUNNING_SOLO] Seed-v2 rejected before tuning by pixel/crop source gate; sealed-v2 was never executed. Active seed-v3: train37 / dev15 / sealed15; text support gate PASS; visual source gate 14 eligible docs / 47 crops; sealed-v3 `95d86b6d...`, bundle `02e378ba...`; sealed policy execution remains zero. Active relay: `../work/2026-09-09-evo35-3d-relay.md`.
  ...
  - [ ] **EVO35.3d.C.VISUAL** [OCR_INDEX_BUILDING] Seed-v3 source gate reduced TRAIN mixed to the five pixel-retrievable amount docs. Build OCR/KURE index for 14 selected docs / 47 eligible crops, then collect TRAIN visual6+mixed5 and DEV visual2+mixed2. SEALED visual/mixed policy execution stays forbidden.
- [ ] **EVO35.3e** [WAITING_3D_VISUAL_AND_RESOURCE_GATE] Train bounded Qwen3.5 next-action SFT adapter in an isolated compatible environment. Freeze base revision/tokenizer/dataset/config hashes and explicit wall-clock/storage budget first; select only on non-golden DEV, never Mini131.
...
Contracts: `specs/evo-harness-qwen35-contract.md`, `specs/evo-training-policy.md`. Active training relay: `../work/2026-09-09-evo35-3d-relay.md`; current executable frontier is EVO35.3d.C.VISUAL. Do not resume the retired Controller queue and do not use Mini131 failures as curriculum.
...
- [ ] **EH2.6** E0/E1 bounded controller를 연결한다. 완료: fact/compare/follow-up의 독립 lane·effect·transition,
  ...
  - [ ] **EH2.6.d** bounded E1 controller와 replay를 구현한다.
    ...
    - [ ] **EH2.6.d2** state+ledger 기반 allowed action과 cross-state `ControllerDecisionReceipt`, c4 effect mint
      ...
      - [ ] **EH2.6.d2.x** `[PARTIAL]` exact transition/effect outcome과 source-owner
        ...
        - [ ] **EH2.6.d2.x.b** `[PARTIAL / B.2_PASS]` ordinal3 fuse·첫 fusion 상태·ordinal4 선택 완료. context/verify 실제 실행과 후속 항목/full matrix는 별도다.
          ...
          - [x] **EH2.6.d2.x.b.2** `[COMPLETED 2026-09-07]` exact revision3→ordinal4 parent/empty 확인 의도/예산 기권 선택. 집중11·관련205·격리216 PASS; full1657 CSV오류1 보존, analytics5 PASS 후 결합·fresh deep 승인. 실제 실행/absence/상태변경 제외.
