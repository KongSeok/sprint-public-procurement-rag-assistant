...
- [ ] **EVO35.3c.R1** [IMPLEMENTED_TESTED_PENDING_LIVE_SOLO] While the frozen `52c2e24` PRE continues unchanged, repair runtime-only policy exhaustion on a separate worktree: bounded policy projection must fit the existing 4096-token policy contract without dropping canonical server evidence, and stagnant duplicate-search loops must become temporarily non-executable. Validate model-free first; rerun only runtime-failure cases with actual Qwen after PRE/model-resource release. Never use answer/gold differences as repair input.
- [ ] **EVO35.3c.R1.VM35** [BLOCKED_AUTH] Deploy the accepted context-projection + duplicate-search-stagnation repair to the Qwen3.5-9B VM runtime without changing model/retrieval/budgets; first inventory the remote repo/branch/runtime, patch on an isolated branch, run focused regression, then restart/switch only the Qwen3.5 experiment service. Current blocker: local gcloud credentials require interactive reauthentication.
