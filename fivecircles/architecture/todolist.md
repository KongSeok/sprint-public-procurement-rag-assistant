...
- [ ] **EVO35.3e** [DATASET_FROZEN_TRAINING_BLOCKED_RESOURCE_ENV_BASE] Train bounded Qwen3.5 next-action SFT adapter in an isolated compatible environment. Frozen TRAIN positive dataset: 157 rows, SHA `b6c7366c...`. Still freeze HF base revision/tokenizer/chat-template, compatible isolated dependency receipt and explicit wall-clock/storage budget first; select only on non-golden DEV, never Mini131.
