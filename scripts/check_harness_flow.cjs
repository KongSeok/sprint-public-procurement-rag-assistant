...
(async () => {
  ...
  try {
    ...
    for (const name of ['Target Flow','Current Implementation Flow','Gap / Priority','Validation']) {
      if (!await page.getByRole('heading',{name,exact:true}).isVisible()) throw Error('heading_missing');
    }
    if (!await page.locator('.legend').isVisible() || await page.locator('table').count() !== 1 || errors.length) throw Error('report_contract_failed');
    ...
    console.log(JSON.stringify({status:'PASS',images:2,tables:1,page_errors:errors.length,mobile_overflow:false}));
