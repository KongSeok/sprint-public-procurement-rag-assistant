...
from midprojectrag.evo_harness.training_authoring_v2 import build_supported_pool,partition_supported_documents,author_supported_cases,audit_support
...
def main(argv=None):
    p=argparse.ArgumentParser(description="Freeze support-audited non-golden Evo seed v2 before tuning.")
    ...
    authored=author_supported_cases(partition["partitions"],exclusion_manifest=exclusion)
    support=audit_support(authored["cases"],authored["targets"],pool["texts"])
    if support["failed_count"]: raise ValueError("nongolden_v2_support_audit_failed")
    ...
    for split in ("sealed_holdout","train","dev"):
        ...
        if split=="sealed_holdout":
            seal={"schema_version":"evo-nongolden-sealed-holdout-v1","split":"sealed_holdout",
                  "document_partition":partition["receipt"]["splits"][split],"support_audit_sha256":support["receipt_sha256"],**receipt}
    ...
    bundle={"schema_version":"evo-nongolden-bundle-v1","pool":pool["receipt"],"partition":partition["receipt"],
            "authoring":authored["receipt"],"support_audit":support,"files":file_receipts,
