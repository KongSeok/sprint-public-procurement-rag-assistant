...
class Mini131ReportTests(unittest.TestCase):
    def test_renders_complete_filterable_report_and_escapes_private_text(self) -> None:
        ...
        self.assertIn('id="api-reproduction-stack"', report)
        self.assertIn("text-embedding-3-small", report)
        self.assertIn("1536 dimensions", report)
        self.assertIn("gpt-5-mini", report)
        self.assertIn("reasoning effort minimal", report)
        self.assertIn("retrieval top-k 10", report)
        self.assertIn("일반/Core/답변형 2000 tokens", report)
        self.assertIn("SDK retry 0", report)
        self.assertIn("git_commit=uncommitted", report)
        self.assertIn("OPENAI_API_KEY", report)
        self.assertNotIn("qwen3.8", report)
        self.assertNotIn("KURE-v1", report)
        self.assertNotIn("vLLM", report)
        self.assertNotIn("NVIDIA L4", report)
