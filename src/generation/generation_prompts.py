...
# - h15(사업명 키워드), g08(나열 항목), g11(참가자격 명확화)에 시도했던
#   프롬프트 지시 3개는 재현성 검증 결과 효과가 없거나 부작용(다른 필수 요소
#   누락)을 일으켜 전부 제거함. 이 케이스들은 이후 answer_generation.py의
#   KEYWORD_COMPLETION_RULES(결정론적 규칙 기반 후처리)로 대체 해결됨 —
#   "긴 목록 중 특정 요소를 반복적으로 놓치는" 유형은 프롬프트보다 후처리가
#   더 안정적이라는 결론.
# ============================================
