...
SCORING_SCHEMA_VERSION = "3.1.1"
_BASE = _V31._BASE
_CITATION_ANYWHERE_RE = re.compile(r"\[\s*근거\s*:\s*([^\]\r\n]+?)\s*\]", re.IGNORECASE)
...
def evaluate(
    ...
    summary.update({
        ...
        "citation_update": "content metrics are separated from final-line format validation",
    })
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="골든셋 결정론적 채점기 v3.1.1")
